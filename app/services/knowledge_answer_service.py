...
def _source_from_result(result: dict[str, Any] | Any) -> dict[str, Any]:
    return {
        ...
        "area": str(_result_value(result, "area", _result_value(result, "area_name", "")) or ""),
        "topic": str(_result_value(result, "topic", _result_value(result, "topic_name", "")) or ""),
...
def _email_source_from_result(result: dict[str, Any] | Any) -> dict[str, Any]:
    return {
        "source": "email",
        "id": str(_result_value(result, "gmail_id", _result_value(result, "id", "")) or ""),
        "subject": str(_result_value(result, "subject", _result_value(result, "title", "Sin asunto")) or "Sin asunto"),
        "sender": str(
            _result_value(
                result,
                "real_sender",
                _result_value(
                    result, "sender", _result_value(result, "original_from", _result_value(result, "subtitle", ""))
                ),
            )
            or ""
        ),
        "date": str(_result_value(result, "received_at", _result_value(result, "date", "")) or ""),
    }
...
def _email_source_label(source: dict[str, Any]) -> str:
    ...
    if source.get("sender"):
        details.append(f"Remitente: {source['sender']}")
    if source.get("date"):
...
def _build_note_context(result: dict[str, Any] | Any, index: int) -> str:
    ...
    area = str(_result_value(result, "area", _result_value(result, "area_name", "")) or "").strip()
    topic = str(_result_value(result, "topic", _result_value(result, "topic_name", "")) or "").strip()
    item_type = str(
        _result_value(result, "type", _result_value(result, "tipo", _result_value(result, "item_type_name", ""))) or ""
    ).strip()
    ...
    indexed_text = _trim_text(_result_value(result, "indexed_text", ""), MAX_FIELD_CHARS)
...
def _build_email_context(result: dict[str, Any] | Any, index: int) -> str:
    subject = str(_result_value(result, "subject", _result_value(result, "title", "Sin asunto")) or "Sin asunto").strip()
    sender = str(
        _result_value(
            result,
            "real_sender",
            _result_value(result, "sender", _result_value(result, "original_from", _result_value(result, "subtitle", ""))),
        )
        or ""
    ).strip()
    recipients = str(_result_value(result, "original_to", _result_value(result, "recipients", "")) or "").strip()
    cc = str(_result_value(result, "original_cc", "") or "").strip()
    date = str(_result_value(result, "received_at", _result_value(result, "date", "")) or "").strip()
    snippet = _trim_text(_result_value(result, "snippet", ""), 900)
    body = _trim_text(_result_value(result, "body_text", _result_value(result, "body", "")), MAX_FIELD_CHARS)
    attachments = _attachment_names_from_value(
        _result_value(result, "attachments_json", _result_value(result, "attachments", ""))
    )
...
def _select_federated_context_results(results: list[dict[str, Any]], max_context_chars: int) -> tuple[list[dict[str, Any]], str]:
    selected: list[dict[str, Any]] = []
    chunks: list[str] = []
    budget = max(1_000, int(max_context_chars or 12_000))
    sorted_results = sorted(results, key=lambda item: _score(item), reverse=True)[:MAX_FEDERATED_RESULTS]
    knowledge_index = 0
    email_index = 0
...
def _build_federated_prompt(
    ...
    knowledge_text = "\n".join(f"- {_source_label(source)}" for source in knowledge_sources) or "- [Sin fuentes Knowledge]"
    email_text = "\n".join(f"- {_email_source_label(source)}" for source in email_sources) or "- [Sin fuentes Email]"
    return f"""
...
def answer_question_from_federated_results(
    ...
    if not cleaned_question or not results:
        ...
        logger.info("FEDERATED_ANSWER: generated chars=%s", len(FEDERATED_NO_INFO_ANSWER))
        return {"answer": FEDERATED_NO_INFO_ANSWER, "sources": {"knowledge": [], "emails": []}}

    selected_results, context = _select_federated_context_results(results, max_context_chars=max_context_chars)
    knowledge_sources = [_source_from_result(result) for result in selected_results if not _is_email_result(result)]
    email_sources = [_email_source_from_result(result) for result in selected_results if _is_email_result(result)]
    logger.info("FEDERATED_ANSWER: sources knowledge=%s emails=%s", len(knowledge_sources), len(email_sources))
    ...
    prompt = _build_federated_prompt(cleaned_question, context, knowledge_sources, email_sources)
    try:
        client = build_openai_client()
    except Exception as exc:  # noqa: BLE001
    ...
    return {"answer": answer, "sources": {"knowledge": knowledge_sources, "emails": email_sources}}
