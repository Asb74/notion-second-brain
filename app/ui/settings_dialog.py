...
class SettingsDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Misc, current: AppSettings, on_save: callable):
        ...
        for idx, (key, label) in enumerate(fields):
            ...
            entry.insert(0, str(getattr(current, key)))
            entry.grid(row=idx, column=1, sticky="ew", padx=6, pady=4)
            self.entries[key] = entry

        ttk.Button(self, text="Guardar", command=self._save).grid(row=len(fields), column=1, sticky="e", padx=6, pady=8)
    ...
    def _save(self) -> None:
        settings = AppSettings(
            ...
            default_estado=self.entries["default_estado"].get().strip(),
            default_prioridad=self.entries["default_prioridad"].get().strip(),
